...
-   **PiPレイアウトの強制表示:** `?test_layout=pip` (メインウィンドウ上でPiP用のレイアウトを確認する場合)

例: `index.html?test_cat=🤝 会議&test_elapsed=3600000&test_layout=horizontal` (1時間経過した会議を横長レイアウトで表示)

## 4. 手動テストケース (PiP)

PiP API は環境依存が強いため、以下の項目を手動で確認します。

| ケース概要 | 確認内容 |
| :--- | :--- |
| PiP ウィンドウの起動 | 📌ボタンを押すと、前面表示の小型ウィンドウが起動すること |
| PiP 内の操作 | カテゴリの切り替え、一時停止、終了が正しく動作すること |
| ウィンドウ間の移動 | PiP ウィンドウを閉じると、アプリ全体がメインウィンドウに自動復帰すること |
| スタイルの適用 | PiP ウィンドウ内でもフォントや色が正しく適用されていること |

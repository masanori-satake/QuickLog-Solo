...
- **背景アニメーションの検証:** アニメーションはタイマーが動作している間のみ描画されます。視覚的な検証（スクリーンショットや Playwright）を行う場合は、業務カテゴリをクリックしてタスクをアクティブにする必要があります。
- **E2E テストの堅牢性:** `BroadcastChannel` 同期やランダム化されたダミー履歴など、非同期処理や動的なデータを含むテストでは、Playwright の `toPass`（リトライアサーション）を使用し、CI 上でのフラッキー（不安定）な結果を回避してください。
- **DB 分離の検証:** `?db=` URL パラメータを使用することで、独立したデータベースインスタンスをテストできます。このとき `BroadcastChannel` も分離されるため、異なるタブ間の干渉を避けることができます。

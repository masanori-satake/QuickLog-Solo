...
- ピン留め中、元のウィンドウは操作の邪魔にならないよう、不透明度を 0 に設定し、`visibility: hidden` を適用して不可視化し、非活性（pointer-events: none）にした上で画面外へ移動・最小化（1x1リサイズ）する。これにより、背後のウィンドウが透けて見えたり操作を妨げたりするのを防ぐ。

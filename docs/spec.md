...
- **データ交換標準:** サイドパネルのインポート/エクスポートは `alarms` と `businessDays` を含みますが、インポート時は既存カテゴリ保護のため `categories` は無視されます。一方、Alarm Editor との連携では完全なデータ移行のため、アラーム、カテゴリ、および稼働日設定の各ストアが同期対象となります。

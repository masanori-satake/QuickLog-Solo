...
- **タイマー表示の高さ標準 (Timer Height):** 3つの高さレベル（'normal' 80px, 'compact' ~53px, 'mini' 40px）をサポート。`SETTING_KEY_TIMER_HEIGHT` 設定に基づき、body クラスと `--timer-height-factor` 変数で制御。'compact' モードではカテゴリ名を省略（ellipsis）、'mini' モードではカテゴリ名を非表示にしタイマーを優先します。
...
- **履歴のカテゴリ・タグ更新ロジック:** 履歴エントリーの編集において、カテゴリを変更した場合は、そのカテゴリに紐付くタグおよび色でエントリーの内容を完全に置き換えます。メモのみの変更など、カテゴリが変わらない場合は既存のタグが維持されます。
...
- **UI 同期の即時性向上:** ユーザーがアプリに復帰した際（visibilitychange/focus）、セッション同期が有効であれば `delayedSync` ロジックを通じて `pullFromCloud()` を明示的に呼び出し、最新のクラウド状態を UI に即座に反映させます。
- **履歴編集 UI 標準:** 履歴編集モーダルは、`height: auto` のヘッダーベースのレイアウトを採用。編集対象が通常のログか停止マーカー（`isManualStop`）かに応じて、タイトル（「履歴の編集」/「終了の編集」）とラベル（「開始時刻」/「終了時刻」）を動的に切り替えます。タスク型ログの編集時は、開始時刻、作業カテゴリ、メモの順に垂直に配置されます。
...
- **重複マーカーの防止:** 停止ボタンの連打等により、同じタイムスタンプを持つ停止マーカーが重複して記録されるのを防ぐロジックを備えています。また、v1.6.2 以降では停止マーカーにも不変な `syncId` (UUID) を付与し、同期時の重複排除を確実なものにしています。
- **履歴ログの表示優先度:** 履歴リストおよび日報生成において、ログの `memo` フィールドが空でない場合は、カテゴリ名よりも `memo` の内容を優先して表示します。
...
- **履歴の再現性:** ログに打刻時点のカテゴリ色を保存し、カテゴリ削除後も当時の色で表示します。また、履歴の正確性を維持するため、打刻時のカテゴリ名やタグはスナップショットとしてログに保存されます。設定でカテゴリ名を変更しても、過去のログには影響しません。
- **データ移行と自動修復:** `initDB` 内の `migrateLogsWithMissingData` 等により、タグや色の情報が欠落しているレガシーなログエントリーに対し、現在のカテゴリ定義に基づいた補完を自動的に行います。
...
- **スケジュールのガードレール:** ユーザー設定の矛盾（例：1日の「前営業日」調整の禁止、末日の「翌営業日」調整の禁止）を UI 側で制御し、実行不能なスケジュールの登録を防止します。また、稼働日がゼロになる設定変更も禁止されています。
...
- **アニメーションのレスポンシブ標準:** タイマーの高さ設定に合わせて、描画内容を動的に調整します。
    - `height` パラメータに基づき、スケール、カウント、座標を再計算。
    - `exclusionAreas` を利用した「スマート配置」により、コンパクト/ミニモードでのタイマーとの重なりを回避。
    - 垂直方向の移動速度などを高さに比例させて調整し、視覚的リズムを維持します。
...
- **TSV/CSV エクスポート標準:**
    - **日報コピー:** ユーザー設定（「なし」 vs 「あり」）に基づき、`endTime` および `duration` カラムを条件付きで含めます。CSV/TSV 形式では「経過時間（Duration）」を「下部」に表示するオプションはサポートされません。
    - **履歴エクスポート:** 履歴全体の CSV エクスポート/インポートは、`id,category,startTime,endTime,tags` の固定 5カラムフォーマットを使用します。
...
### 5.4. セッション連携 (Sync) [β版]
- **目的:** 複数端末間での設定、カテゴリ、および直近の履歴（最大50件）の同期。
- **技術仕様:** `chrome.storage.sync` を利用。8KB の容量制限を回避するため、履歴ログを 5 つのチャンクに分割して保存。
- **堅牢な同期ロジック:** ログの識別に `crypto.randomUUID()` による不変な `syncId` を使用。これにより、一方で開始時刻や終了時刻が修正された場合でも、他方の端末で重複が発生することなく正しく変更が反映されます。また、`clientId` による無限ループ防止機能を備えています。
- **削除と編集の伝播 (v1.6.2+):**
    - 削除されたログの `syncId` は `SETTING_KEY_DELETED_SYNC_IDS` 内の「トムストーン（墓石）」リストに一定期間保持され、同期時の再混入を防止します。
    - カテゴリやメモの変更は、リモートログが同じ `endTime` を持っていても、ローカルの変更を上書き・統合する `reconstructTimeline` ロジックによって端末間で伝播されます。
- **実験的機能 (β版):** 本機能は現在「試験的な β テスト」の位置づけです。ブラウザの同期機能（Google アカウント等）に依存するため、異なるブラウザベンダー間（Chrome と Edge など）での同期は行えません。
...
- **デプロイ戦略とパス解決 (Vercel):** `cleanUrls: false` および `trailingSlash: true` を明示的に設定。サブディレクトリ（`projects/app/` 等）から `shared/` アセットを解決するため、`vercel.json` にて Rewrite ルールを適用しています。サブプロジェクト（`/alarm-editor/` 等）へのアクセスは `redirects` によりクリーンな URL を維持しつつ、物理ディレクトリへマッピングされます。
...
- **バージョンバンプと CI 強制:** `projects/app/` または `shared/` 内の変更は、CI の `scripts/verify_version_impact.py` をパスするためにバージョンバンプを必要とします。機能追加 (`feat:`) コミットにはマイナーバンプ、修正 (`fix:`) コミットにはパッチバンプが必須です。`scripts/bump_version.py` を使用して、`package.json`、`package-lock.json`、`projects/app/version.json`、`projects/app/manifest.chrome.json`、および README.md のバージョンバッジを一括同期します。
...
- **セマンティック UI カラー標準:** 視覚的な一貫性とアクセシビリティを確保するため、セマンティックな CSS 変数（`--app-color-sunday` 等）の使用を徹底します。エラーではない箇所に `error` カラーなどの機能色を流用することは禁止されています。
...
- **ベータ機能の UI 標準:** ベータ版として指定された機能（セッション同期など）は、ラベルの末尾にテキスト形式で `(β)` または `(Beta)` を付与します。Symbolic なバッジ（`<span class="beta-badge">`）は、UI の簡潔さを保つため使用しません。
...
- **UI 構成:** 360px 固定幅のマスターリストと柔軟な詳細ペインによる Master-Detail レイアウトを採用。詳細ペインの「基本設定」と「スケジュール設定」は 2カラム配置（24px 間隔）です。
...
- **UI の視認性:** アラームの曜日選択など、アクティブな状態を示す UI 要素は、非選択状態と明確に区別できるよう、コントラストの高い背景色（Solid background）を使用しなければなりません。
- **独立データベース:** `QuickLogSoloAlarmEditorDB` という隔離された IndexedDB を使用し、メインアプリのデータへの干渉を防止。起動ごとにストアをクリアして初期化することで、常にクリーンな編集環境を維持します。
- **Undo/Redo のスコープと永続性:** アラーム設定と稼働日設定のみを Undo/Redo の対象とし、言語やテーマの設定は安定性のため対象外とします。履歴操作ごとに `saveAllAlarms` を介して IndexedDB へ即座に永続化します。
...
- **ポリシー見出しの国際化:** ポリシーセクションの見出しは、`policy-[feature]-short` 形式（例: `policy-no-tracking-short`）の標準化された i18n キーを使用します。全 8 言語において統一された見出しを提供します。

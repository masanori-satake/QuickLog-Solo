...
- **日報コピー:**
    ...
    - 絵文字の除去、終了時刻の表示、経過時間の表示位置（右または下）、5/10分単位の時刻丸め（Time Adjustment）を設定可能。
...
- **安定性の向上 (v0.10.0):** 起動時の負荷による誤作動を防ぐため、180フレーム（約3秒）の「ウォームアップ期間」を導入し、この期間内の遅延はガードレールのカウント対象外とする。
...
- **特徴:**
    - エディタ: 行番号表示、シンタックスハイライト、オートインデント、検索・置換、折り返し切り替え。
    ...
    - **強制表示フラグ (ignoreExclusion):** 検証用モジュール向けに、UI要素によるマスキング（ドットの非表示化）を強制的にバイパスし、キャンバス全体に描画を行うフラグをサポート。これにより、オーバーレイ要素の不透明度に関わらずアニメーションの動作を確実に目視確認できます。

...
- **QR 設定取り込み:** カテゴリ・アラームの全件検証後、設定の更新とストアの置換を単一トランザクションで実施し、失敗時の部分更新を防ぐ。成功後に他画面へ同期を通知する。読み取りは QR 対応の `BarcodeDetector` がある環境に限定し、非対応環境ではカメラを起動せず案内を表示する。同じモーダルで画像を選び直した場合は、古い画像の読み込み・デコード結果を破棄し、設定などの上書きを防ぐ。

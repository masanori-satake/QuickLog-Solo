...
- **パス・ポータビリティ標準:** ローカル、GitHub Pages、拡張機能の全環境で動作させるため、アセット参照は常に相対パス（先頭スラッシュなし）を使用します。
...
- **「不明（Unknown）」タスクの挿入抑制 (v1.6.5):** タイムライン再構築時、記録漏れを防ぐために隙間を「不明」タスクで埋める処理が行われますが、以下のケースでは挿入が抑制されます。
    - **上書き同期モード:** 端末間同期で「他端末のデータで上書き」を選択した場合、他端末のデータ整合性を優先し、隙間を埋めずにそのまま再現します。
    - **手動終了後:** 手動で「終了」ボタン（⏹）を押した際の停止マーカー（`isManualStop: true`）が隙間の開始点より前に存在し、かつその間に他の実タスクがない場合、その隙間（日を跨ぐ場合や、他のゼロ期間マーカーで分割された場合を含む）には「不明」タスクを挿入しません。これにより、一日の終わりから翌日の開始までの空白が適切に維持されます。
...
- **QR 設定インポート:** 保存前に全カテゴリ・アラームの型、必須 ID、ID の重複を検証します。途中の失敗による既存データの消失を防ぐため、設定と対象ストアの置換は単一トランザクションで確定し、成功後に同期通知を送ります。分割 QR で省略されたカテゴリ・アラームは既存データを保持し、明示的な空配列は対象ストアを空にします。各エクスポート QR は必要なストアを個別に取得し、取得失敗は該当 QR のみに表示します。 画像選択の有効性はカメラ起動とは独立して管理します。画像の再選択やスキャナーの開閉で古い結果が保存されないよう、進行中のインポートはDB接続待ちからトランザクション確定まで中止可能にします。
...
- **PWA の保存先:** `chrome.storage.sync` が使えない場合は、同期項目ごとの `localStorage` キーに保存し、タブ間で無関係な項目を上書きしない。旧共有キーは読み取り専用で参照し、削除は個別キーの削除印で表す。書き込みエラーは Promise と `chrome.runtime.lastError` で呼び出し元に伝える。
...
- **デプロイ戦略とパス解決 (GitHub Pages):** Vercel からの移行にあたり、外部ホスティングの設定と依存を減らし、ソース管理と公開を GitHub に集約できるため、GitHub Pages による完全静的デプロイを採用。ルートディレクトリから全サブプロジェクト（`projects/app/`, `projects/category-editor/` 等）を同一オリジンで配置・配信します。
...
- **ポリシー・ガードの自動化:** CI (`tests-and-lint.yml`, `osv-scan.yml`) および `pre-commit` にて `No Warranty` 表記、`Local Only` 関連文言、`google-site-verification`、`innerHTML` 例外の範囲を自動検証し、設計思想と実装の乖離を早期に検出します。
...
- **ウェブベース移行 (v0.32.0):** GitHub Pages ホストのウェブ版へ移行。データ連携はクリップボード (NDJSON) を介して実施。`?from=app` パラメータにより「戻る」リンクを非表示にする制御を維持。

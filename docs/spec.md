...
- **工数集計 (Time Aggregation):** カテゴリ別の合計時間に加え、タグ別の合計（`#tag | N min`）も算出し、多次元的な時間分析をサポートする。
...
- **日報作成 UI:** カレンダーから特定の日を選択可能。日付表示部分（`tabindex="0"`）で `ArrowUp`/`ArrowDown` キーを操作することで、記録がある日のみをスキップしながら選択できる。

...
- ページネーション（1ページ8項目）をサポートし、多数のカテゴリがあってもレイアウトを維持。マウスホイールで切り替え可能。
...
### F. ピン留め機能 (Document Picture-in-Picture)
- PWAとしてインストールされている場合、Document Picture-in-Picture APIを利用してアプリを常に最前面に表示できる。
- ピン留め中、元のウィンドウは操作の邪魔にならないよう、不透明度を 0 に設定し、`visibility: hidden` を適用して不可視化し、非活性（pointer-events: none）にした上で画面外（右下端）へ移動・最小化（1x1リサイズ）する。また、タイトルバーを空文字（スペース）に設定し、タスクバー等での存在感を最小限に抑える。
- ピン留めを解除（PiPウィンドウを閉じる）すると、元のウィンドウ位置とサイズ、不透明度、操作性が即座に復元され、手前にフォーカスされる。
- **安全策:** 起動時にウィンドウが画面外（または不可視状態）にある場合は、自動的に画面内の視認可能な位置（100, 100）へ移動・復元し、ユーザーが操作不能になるのを防ぐ。
...
- **レスポンシブデザイン:** ウィンドウサイズに合わせてレイアウトが最適化される。
    ...
    - **ピン留め（PiP）モード:** 最小限の要素のみを表示するコンパクトレイアウト。ウィンドウサイズに合わせて中身が伸縮するレスポンシブ設計を採用。

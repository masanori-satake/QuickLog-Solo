...
- **ローカルファイル同期 (Backup):**
    - File System Access API を使用した、特定ディレクトリへの履歴自動保存。
    - **NDJSON Partitioning:** 1日1ファイルの形式で保存され、単一ファイルの破損が全履歴に波及するのを防ぐ設計。
    - **レジリエンス:** ブラウザのキャッシュクリア等で IndexedDB が消失しても、フォルダを再指定することで履歴を復旧可能。
    - **ステータス表示:** ヘッダーの円形インジケーターにより、同期状況（成功/Dirty/エラー）を視覚的にフィードバック。
...
- **透過性の維持:** アニメーションがアクティブな際、前面要素の背景色を透過させるために CSS で `!important` を使用した強制上書きを行っています。
...
- **特徴:**
    ...
    - **リセット挙動:** サンプル切り替え時はメタデータやプレビュー設定がリセットされますが、`exclusionStrategy` は新しく読み込まれたモジュールの設定と同期されます。

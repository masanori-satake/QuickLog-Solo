...
- **QR 設定インポート:** 保存前に全カテゴリ・アラームの型、必須 ID、ID の重複を検証します。途中の失敗による既存データの消失を防ぐため、設定と対象ストアの置換は単一トランザクションで確定し、成功後に同期通知を送ります。 画像選択の有効性はカメラ起動とは独立して管理します。画像の再選択やスキャナーの開閉で古い結果が保存されないよう、進行中のインポートはDB接続待ちからトランザクション確定まで中止可能にします。

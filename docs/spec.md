...
- **スクリプト命名規則:** `verify_*.py` および `reproduce_*.py` は一時的な検証用として扱われ、Git 管理から除外されます。永続的な監査スクリプトには `audit_*.py` 等の名称を使用します。
...
- **CI 言語監査への対応:** 既存の PR タイトルに日本語が含まれず CI 監査に失敗した場合、新しいブランチ名で再度 `submit` を行うことで、正しいタイトル・説明文を持つ新しい PR を強制的に生成し、監査をパスさせます。

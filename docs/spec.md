...
- **Vercel デプロイとパス解決:** サブディレクトリ（`projects/app/` 等）から `shared/` アセットを解決するため、`vercel.json` にて Rewrite ルールを適用しています。
...
- **経過時間フォーマット:** 
    - **レポート出力:** `formatLogDuration` により、1時間以上の場合は「1h 5m」（10分未満はスペースあり）、「1h15m」（10分以上はスペースなし）という視覚的なリズムを持った形式で出力されます。

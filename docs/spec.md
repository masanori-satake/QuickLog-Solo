...
- ピン留め中、元のウィンドウは操作の邪魔にならないよう、モニタの端へ移動し、最小サイズ（100x100等、ブラウザの許容最小値）にリサイズされる。
- ピン留めを解除（PiPウィンドウを閉じる）すると、元のウィンドウ位置とサイズに復元され、手前にフォーカスされる。
- **安全策:** 起動時にウィンドウが画面外にある場合は、自動的に画面内の視認可能な位置（100, 100）へ移動し、ユーザーが操作不能になるのを防ぐ。

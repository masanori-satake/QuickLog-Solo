...
### D. データライフサイクル
- **保持期間:** 直近30日間。
- ログ表示機能は存在しないが、データはIndexedDBに保持され、CSVエクスポートやコピー機能で利用可能。
...
## 4. UI/UX デザイン・透明性設計
- **レイアウト:** デスクトップ端に馴染む「縦長スリム」かつ、すべての要素がスクロールなしで収まるコンパクトなデザイン。
...
- **背景アニメーション:** 実行中エリアの背景が1分周期で「白背景 ↔ アクセントカラー」と右から左へ塗られていく。
- **確認ダイアログ:** ブラウザ標準の `window.confirm` を廃止し、アプリ内にデザインを統一したカスタムダイアログを実装。
- **レスポンシブデザイン:** ウィンドウサイズに合わせてレイアウトが最適化される。横幅が600px以上でカテゴリ一覧とタイマーが横に並ぶ2カラムレイアウトになり、すべての要素がスクロールなしで1画面に収まるよう調整されている。

### 配色ポリシー
- ユーザーは「文字色」のみをプリセットから選択。背景色は視認性を確保するために自動的に選択される。これにより、色の組み合わせに悩む時間を削減し、本来の作業に集中できるようにする。

### 状態表示
- 作業状態をオーディオプレイヤーに準拠したグリフで表現。
    - 実行中: ▶ (Green)
    - 待機中/一時停止中: ⏸ (Orange)
    - 停止中: ⏹ (Red)

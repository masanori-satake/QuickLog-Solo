...
- **Vercel デプロイとパス解決:** サブディレクトリ（`projects/app/` 等）から `shared/` アセットを解決するため、`vercel.json` にて Rewrite ルールを適用しています。また、`cleanUrls: false` および `trailingSlash: true` を明示的に設定し、安定したルーティングを確保します。
- **透明性レポート標準:** `transparency.html` は、ユーザーの懸念（データ収集への不安など）を平易な言葉で解決する「懸念優先（Concern-First）」構造を採用しています。10年の長期利用を想定した設計や、Google OSV-Scanner を用いたプロダクション依存関係ゼロの技術的証明を、全 8 言語で提示します。

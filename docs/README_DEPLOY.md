...
graph LR
    ...
    Registry --> CheckVersion[バージョン整合性確認<br/>scripts/check_version.py]
    CheckVersion --> Zip[パッケージ作成<br/>scripts/create_package.py]
    Zip --> Deploy[サイト公開]
```
...
2. 以下の情報を取得：
   ...
   - **Org ID**: Settings > General (Personal Account ID または Team ID)
3. **Access Token** を発行（Account Settings > Tokens）。

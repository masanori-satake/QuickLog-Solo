...
#### static metadata
モジュールの情報オブジェクト。利用者が利用に際して必要とする情報や、管理運営者の立場でアニメーションモジュールに対する作業判断に必要とする情報が含まれる。
...
- `rewindable`: (オプション) 巻き戻し（過去の `elapsedMs` での描画）が可能かどうかのフラグ。開発環境が処理内容を決定するために使用します。
...
描画エンジンへの動作指示設定。アニメーションエンジンが自身の振る舞いを決定するために必要とする情報が含まれる。
- `mode`: 描画モードの指定。

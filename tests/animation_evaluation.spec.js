...
test.describe('Animation Quality Evaluation', () => {
    ...
    for (const id of animationIds) {
        test(`Evaluating animation: ${id}`, async ({ page }) => {
            ...
            const sustainedActivity = stats.samples.some(
                (s) => s.elapsed >= EVAL_CONFIG.SUSTAINED_ACTIVITY_CHECK_MS && s.changed > 0
            );
            ...
            expect(stats.firstActivityTime, `Animation ${id} did not start within threshold`).toBeLessThanOrEqual(
                EVAL_CONFIG.INITIAL_ACTIVITY_THRESHOLD_MS
            );
            expect(sustainedActivity, `Animation ${id} did not show sustained activity`).toBe(true);
            expect(avgDots, `Animation ${id} is too sparse (avg dots: ${avgDots})`).toBeGreaterThanOrEqual(
                EVAL_CONFIG.MIN_AVERAGE_DOTS
            );
            expect(
                avgChangeRate,
                `Animation ${id} is too static (avg change rate: ${avgChangeRate})`
            ).toBeGreaterThanOrEqual(EVAL_CONFIG.MIN_CHANGE_RATE);
        });
    }
});

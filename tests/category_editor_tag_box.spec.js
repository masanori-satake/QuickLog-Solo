...
test.describe('Category Editor Tag Box', () => {
    ...
    test('Dragging a tag from Tag Box to input adds it to the category', async ({ page }) => {
        ...
        await page.evaluate(() => {
            const targetTag = Array.from(document.querySelectorAll('#global-tag-list .tag-pill')).find(
                (el) => el.textContent === 'targetTag'
            );
    ...
    test('Duplicates are ignored when dragging from Tag Box', async ({ page }) => {
        ...
        await page.evaluate(() => {
            const tagA = Array.from(document.querySelectorAll('#global-tag-list .tag-pill')).find(
                (el) => el.textContent === 'tagA'
            );
            const tagInput = document.querySelector('#tag-input');
    ...
    test('Tag Box updates when category tags change', async ({ page }) => {
        await page.evaluate(() => {
            window.state.categories = [{ name: 'CatA', color: 'primary', tags: 'tagA', animation: 'default' }];

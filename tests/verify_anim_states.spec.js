...
test('capture plant animation at 85%', async ({ page }) => {
  ...
  await page.goto('http://localhost:8080/src/app.html?test_cat=🌱%20アイデア出し・企画立案&test_elapsed=102000');
...
test('capture matrix animation', async ({ page }) => {
  await page.goto('http://localhost:8080/src/app.html?test_cat=💻%20開発・プログラミング&test_elapsed=30000');

...
- **Visual Healing（視覚的癒やし）:** 20種類のLCDドットマトリクス風アニメーションを搭載。「1秒の重み」を緩やかな変化で表現し、作業中のストレスを軽減する心地よい体験を提供します。
- **タグ別集計:** カテゴリにタグを紐付けることで、複数のカテゴリにまたがるプロジェクト横断の工数集計を一瞬で行えます。ヘッダーの「📊」ボタンから利用可能です。
- **アラーム・通知機能:** 指定した時刻にメッセージを表示し、作業を自動的に終了・一時停止・開始できます。デフォルトで 23:59 に終了する設定が有効になっており、止め忘れを防止して翌日の記録をクリーンに開始できるようサポートします。
...
- **背景アニメーション:** 20 種類の LCD ドットマトリクス風アニメーションを搭載。

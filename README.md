...
- **ローカルファイルバックアップ:** 指定したローカルフォルダへの自動同期に対応。ブラウザのキャッシュクリア等による予期せぬデータ消失から記録を守ります（File System Access API を利用）。
- **徹底したプライバシーと透明性:**
...
- **QL-Animation Studio (β版):** ブラウザ上でオリジナルの背景アニメーションを作成・テストできる開発環境を公開。自分の作品を製品に組み込むことも可能です。

export default {
    title: 'QuickLog-Solo',
    'copy-report-btn': '复制日报',
    'copy-report-title': '复制日报',
    'tag-aggregation-title': '标签汇总',
    'copy-aggregation-btn': '复制统计',
    tags: '标签',
    'no-tags': '(无标签)',
    'total-work-time': '工作时间',
    settings: '设置',
    delete: '删除',
    duplicate: '创建副本',
    'btn-copy': '复制',

    // Auto Stop & Page Break
    'page-break': '换页',
    'btn-add-page-break': '添加换页',
    'btn-add-category': '添加分类',
    'btn-add-custom-anim': '添加自定义动画',
    'confirm-delete-page-break': '是否删除此换页？',

    // Tooltips
    'tooltip-copy-report': '复制日报到剪贴板',
    'tooltip-copy-aggregation': '按标签汇总结果并复制到剪贴板',
    'tooltip-settings': '打开设置',
    'tooltip-drag-handle': '拖动以排序',
    'tooltip-delete-category': '删除此分类',
    'tooltip-delete-history': '删除此历史记录',
    'tooltip-language-select': '切换显示语言',
    'tooltip-theme-select': '切换浅色/深色模式',
    'tooltip-font-select': '选择显示字体',
    'tooltip-font-weight-select': '选择字体粗细',
    'tooltip-category-layout-select': '选择分类按钮布局',
    'tooltip-animation-select': '选择全局背景动画',
    'tooltip-pause-animation-select': '选择待机背景动画',
    'tooltip-pause-theme-select': '选择待机主题颜色',
    'tooltip-timer-height-select': '选择计时器区域高度',
    'tooltip-status-running': '正在运行',
    'tooltip-status-paused': '已暂停',
    'tooltip-status-stopped': '已结束',
    'tooltip-calendar': '选择日期',
    pause: '暂停',
    resume: '继续',
    stop: '停止',
    'tab-general': '常规',
    'tab-categories': '分类',
    'tab-maintenance': '维护',
    'tab-backup': '备份',
    'tab-alarms': '闹钟',
    'tab-about': '关于',

    // Alarms Tab
    'business-days': '工作日',
    'tooltip-edit-business-days': '编辑',
    'alarm-editor-title': '闹钟编辑器',
    'btn-launch-alarm-editor': '编辑',
    'alarm-label-type': '执行时机',
    'alarm-type-daily': '每天',
    'alarm-type-daily_business': '每天 (仅工作日)',
    'alarm-type-weekly': '指定星期',
    'alarm-type-monthly_date': '每月指定日期',
    'alarm-type-monthly_end_relative': '每月从月末起算',
    'alarm-label-holiday-adjustment': '非工作日调整',
    'alarm-adj-none': '不调整',
    'alarm-adj-prev_business_day': '提前 (上一个工作日)',
    'alarm-adj-next_business_day': '延后 (下一个工作日)',
    'alarm-adj-skip': '跳过 (不执行)',
    'alarm-adj-desc-none': '即使指定日期是非工作日也照常执行。',
    'alarm-adj-desc-prev_business_day': '如果指定日期是非工作日，则在之前的上一个工作日执行。',
    'alarm-adj-desc-next_business_day': '如果指定日期是非工作日，则在之后的下一个工作日执行。',
    'alarm-adj-desc-skip': '如果指定日期是非工作日，则跳过该月/该周的执行。',
    'label-day': '日',
    'label-before-end-1': '距离月末',
    'label-before-end-2': '天 (0为月末当天)',
    'back-to-app': '返回应用',
    'alarm-setting-description': '在指定时间显示消息，并根据需要自动执行“结束”、“暂停”或“开始”操作。',
    'alarm-note-extension-only': '* 闹钟功能仅在作为浏览器扩展程序安装时有效。预览版中不会执行通知和自动操作。',
    'alarm-label-enabled': '启用',
    'alarm-label-time': '时间',
    'alarm-label-message': '消息',
    'alarm-label-action': '动作',
    'alarm-label-category': '启动分类',
    'alarm-action-none': '仅通知',
    'alarm-action-stop': '结束任务',
    'alarm-action-pause': '暂停任务',
    'alarm-action-start': '开始分类',
    'alarm-placeholder-message': '输入消息...',
    'alarm-tooltip-confirmation': '在点击确定按钮前保持通知，并在确认后执行操作',
    'toast-alarm-saved': '闹钟已保存',
    'btn-test-notification': '测试通知',
    'test-notification-message': '这是一条测试通知。它工作正常。',

    'setting-enabled': '已开启 (ON)',
    'setting-disabled': '已关闭 (OFF)',
    'setting-language': '语言',
    'lang-auto': '浏览器语言',
    'lang-ja': '日语',
    'lang-en': '英语',
    'setting-theme': '主题',
    'theme-system': '跟随系统',
    'theme-light': '浅色模式',
    'theme-dark': '深色模式',
    'setting-font': '字体',
    'setting-font-weight': '字体粗细',
    'font-weight-normal': '标准',
    'font-weight-medium': '中等',
    'font-weight-bold': '粗体',
    'font-weight-heavy': '极粗',
    'setting-category-layout': '分类按钮布局',
    'category-layout-2x8': '2列 x 8行',
    'category-layout-2x4': '2列 x 4行',
    'setting-animation-default': '标准背景动画',
    'setting-pause-animation': '待机背景动画',
    'setting-pause-theme': '待机主题',
    'setting-timer-height': '计时器区域高度',
    'timer-height-normal': '标准 (100%)',
    'timer-height-compact': '略低 (2/3)',
    'timer-height-mini': '更低 (1/2)',
    'setting-animation-by-category': '背景动画设置',
    'setting-history-csv': '工作历史 (CSV)',
    'btn-export-csv': '导出',
    'btn-import-csv': '导入',
    'placeholder-new-category': '新分类...',
    'placeholder-tags': '标签 (逗号分隔)',
    'btn-export-json': '复制 (导出)',
    'btn-import-json': '粘贴 (导入)',
    'import-overwrite': '覆盖',
    version: '版本',
    developer: '开发者',
    'about-stats-logs': '工作历史数量',
    'about-stats-categories': '分类数量',
    'about-description':
        'QuickLog-Solo 是一款注重隐私的极简工作笔记工具。数据保存在您浏览器的 IndexedDB 中（执行备份时也会保存到本地文件系统），且绝不上传服务器。通过 GitHub Actions 的严格依赖校验和 Google OSV-Scanner 的持续审计，确保高度的透明性与安全性。',
    'about-disclaimer':
        '[免责声明] 这是一个个人开源项目，按“原样”提供，不附带任何形式的保证。开发者不对因使用本软件而产生的任何损害承担责任。请自行承担使用风险。',
    // Maintenance - Backup & Restore
    'maintenance-backup-title': '备份与恢复',
    'backup-start': '开始备份',
    'backup-execute': '备份',
    'backup-change-dir': '更改目标位置',
    'restore-btn': '恢复',
    'backup-status-success': '备份完成',
    'backup-status-failed': '备份失败',

    // Maintenance - Restore Dialog
    'confirm-restore': '是否恢复数据？',
    'confirm-restore-desc': '所有当前数据将被删除并用备份数据覆盖。此操作无法撤销。',

    // Maintenance - Restore Messages
    'restore-success': '恢复已完成',
    'restore-error-invalid-folder': '不是有效的备份文件夹。缺少必需文件。',
    'restore-error-write-failed': '恢复过程中发生错误。数据可能处于不完整状态。',
    'restore-skipped-records': '跳过了 {count} 条无效记录',

    // Maintenance - Delete/Initialize
    'maintenance-delete-title': '删除/初始化',
    'maintenance-clear-logs': '工作历史',
    'maintenance-clear-categories': '分类',
    'maintenance-clear-settings': '设置',
    'maintenance-clear-alarms': '闹钟',
    'maintenance-clear-animations': '自定义动画',
    'maintenance-delete-execute': '删除/初始化',
    'confirm-delete-initialize': '以下数据将被删除/初始化：\n{items}\n\n此操作无法撤销。确定要执行吗？',
    'maintenance-delete-success': '已删除/初始化所选项目',
    'maintenance-delete-error': '删除 {item} 时发生错误',

    'btn-clear-logs': '删除所有工作历史',
    'maintenance-sync-pull': '重新同步历史记录',
    'btn-sync-pull': '重新同步历史记录',
    'maintenance-sync-clear-cloud': '从所有同步设备中删除历史',
    'btn-sync-clear-cloud': '从所有同步设备中删除所有工作历史',
    'maintenance-reset-all': '重置设置与分类',
    'maintenance-reset-all-desc': '工作历史将被保留。',
    'btn-reset-all': '重置分类与设置',
    'btn-reset': '重置',
    'btn-initialize-all': '全部初始化',
    'btn-initialize': '初始化',
    'maintenance-reset-settings': '仅重置设置',
    'maintenance-reset-settings-desc': '工作历史与分类将被保留。',
    'btn-reset-settings': '仅重置设置',
    // Backup Tab
    'btn-backup-start': '开始备份',
    'btn-backup-run': '执行备份',
    'btn-backup-grant-run': '授予权限并保存备份',
    'btn-backup-change-dir': '更改备份目录',
    'backup-status-syncing': '同步中...',
    'backup-last-time': '上次备份时间',
    'backup-directory': '保存目录',
    'backup-not-selected': '(未选择)',
    'backup-file-count': '文件数量',
    'backup-file-count-unit': '天',
    'backup-last-time-label': '上次备份',
    'backup-file-count-label': '历史天数',
    'backup-description-new':
        '建议定期备份，以防因浏览器缓存清理导致 IndexedDB 数据丢失。有了备份数据，您还可以顺利迁移到其他浏览器。',
    'toast-backup-completed': '备份完成',

    'confirm-end-task': '确定要结束任务吗？',
    'history-edit-title': '编辑历史',
    'history-edit-stop-title': '编辑停止',
    'history-action-title': '历史操作',
    'history-edit-start-time': '开始时间',
    'history-edit-end-time': '结束时间',
    'history-edit-category': '分类',
    'history-edit-memo': '备注',
    'history-edit-out-of-range': '指定的时间超出范围。',
    'placeholder-memo': '输入备注...',
    'confirm-delete-history': '您要删除这条历史记录吗？',
    'btn-apply': '应用',
    'btn-edit': '编辑',
    'btn-split': '分割',
    'btn-delete': '删除',
    'confirm-cancel': '取消',
    'confirm-discard-changes': '更改未保存。是否放弃更改并关闭？',
    'confirm-delete-category': '删除分类 "{name}"？\n（旧工作历史中的分类颜色将消失）',
    'confirm-delete-selected': '是否删除所选的 {count} 个项目？',
    'confirm-import-overwrite': '删除所有现有分类并覆盖？',
    'confirm-export-csv': '工作历史将导出为 CSV。当前任务将结束。是否继续？',
    'confirm-import-csv': '将从 CSV 导入工作历史。当前任务将结束。是否继续？',
    'confirm-clear-logs': '将删除所有工作历史。当前任务将结束。是否继续？',
    'confirm-sync-pull': '将删除本地工作历史并从其他设备重新获取。当前任务将结束。是否继续？',
    'confirm-sync-clear-cloud': '将从所有同步设备（而不只是本设备）中删除工作历史。当前任务将结束。是否继续？',
    'confirm-reset-all': '将重置分类与设置。当前任务将结束。是否继续？（工作历史保留）',
    'confirm-reset-settings': '将重置设置。当前任务将结束。是否继续？（工作历史与分类保留）',
    'alert-invalid-category': '分类名无效。（50字以内，不可使用 "{idle}"）',
    'alert-duplicate-category': '已存在同名分类。',
    'alert-import-error': '导入分类失败。',
    'alert-init-error': '应用初始化失败。',
    'toast-copied': '已复制！',
    'toast-done': '已完成！',
    'toast-done-with-reopen-msg': '应用成功！如果设置面板已打开，请将其关闭并重新打开以应用新动画。',
    'toast-imported': '导入完成。',
    'toast-cat-imported': '分类已导入',
    'toast-deleted': '删除完成',
    'no-logs-for-day': '该日无工作历史',

    'notification-btn-ok': '确定',
    'notification-btn-close': '关闭',

    'import-err-fatal': '文件格式不正确。导入已中止。',
    'import-err-partial': '在共 {total} 行中，有 {errorCount} 行已损坏。是否仅导入正常的 {validCount} 行？',
    'import-err-field': '部分数据不完整（例如：颜色无效）。是否应用默认值并导入全部，或者跳过无效行？',
    'import-btn-import-valid': '仅导入有效行',
    'import-btn-apply-fallback': '应用默认值',
    'import-btn-skip-invalid': '跳过无效行',
    'import-btn-abort': '中止',

    'backup-err-0byte': '发现一个空的备份文件 ({name})。这可能表示存在错误。是否忽略此文件并继续？',
    'backup-btn-ignore-continue': '忽略并继续',
    'backup-btn-abort-investigate': '中止并调查',

    'backup-err-locked': '文件被另一个程序锁定或您没有权限。请关闭其他应用程序并重试。',
    'backup-err-not-found': '未找到备份目录。请重新选择目录。',
    'backup-err-unknown': '发生未知错误: {message}',
    'toast-backup-failed-detail': '备份失败。原因: {reason}',

    'day-names': ['日', '一', '二', '三', '四', '五', '六'],
    'day-0': '日',
    'day-1': '一',
    'day-2': '二',
    'day-3': '三',
    'day-4': '四',
    'day-5': '五',
    'day-6': '六',
    'idle-category': '(闲置)',
    'idle-category-log': '(闲置)',
    'category-unknown': '未知',
    'status-stopped-name': '(已停止)',
    'font-system': '系统字体',
    'anim-default': '标准',
    'anim-none': '无',
    'anim-author-label': '作者',
    'anim-unknown-author': '未知',

    // Report Settings
    'report-emoji-keep': '有',
    'report-emoji-remove': '无',
    'report-endtime-none': '无',
    'report-endtime-show': '有',
    'report-duration-none': '无',
    'report-duration-right': '右侧',
    'report-duration-bottom': '下方',
    'report-format-title': '格式',
    'report-format-markdown': 'Markdown',
    'report-format-wiki': 'Wiki 标记',
    'report-format-html': 'HTML 表格',
    'report-format-csv': 'CSV',
    'report-format-tsv': 'TSV',
    'report-format-text-table': '文本 (表格形式)',
    'report-format-text-plain': '文本 (无框)',
    'report-emoji-title': '移除表情',
    'report-endtime-title': '结束时间',
    'report-duration-title': '所需时间',
    'report-header-time': '时间',
    'report-header-category': '分类',
    'report-adjust-title': '时间调整',
    'report-adjust-none': '无',
    'report-adjust-5m': '5分钟间隔',
    'report-adjust-10m': '10分钟间隔',

    'init-cat-dev': '💻 开发与编程',
    'init-cat-meeting': '🤝 团队会议',
    'init-cat-research': '🔍 调查与研究',
    'init-cat-admin': '行政事务、邮件处理 📝',
    'init-cat-focus': '🔥 深度专注任务',
    'init-cat-skill': '📚 自我提升',
    'init-cat-idea': '💡 构思与规划',
    'init-cat-break': '☕ 冥想与放松',
    'demo-warning': '💡 演示工作历史（使用前请在维护中清除）',
    'init-cat-client': '📞 客户沟通',
    'init-cat-doc': '📝 文档与报告',
    'init-cat-design': '🎨 设计/UI/UX',
    'init-cat-bug': '🐛 Bug 修复',
    'init-cat-release': '🚀 发布与部署',
    'init-cat-tool': '🛠 工具维护与自动化',
    'init-cat-schedule': '🗓 日程与任务管理',
    'init-cat-chat': '💬 聊天/Slack/Teams',
    'init-cat-wiki': '📖 文档/Wiki 更新',
    'init-cat-qa': '🧪 测试/QA',
    'init-cat-sales': '💼 销售/提案',
    'init-cat-arch': '🏗 架构设计',
    'init-cat-sec': '🔐 安全/审计',
    'init-cat-data': '📊 数据分析/SQL',
    'init-cat-wfh': '🏠 居家办公环境',
    'init-cat-move': '🚶 出差/移动',

    // Initial Tags
    'init-tag-dev': '开发, 设计',
    'init-tag-meeting': '会议',
    'init-tag-research': '调查',
    'init-tag-admin': '事务',
    'init-tag-focus': '专注',
    'init-tag-skill': '自我提升',
    'init-tag-idea': '策划',
    'init-tag-break': '休息',
    'init-tag-client': '客户服务',
    'init-tag-doc': '文档, 报告',
    'init-tag-design': '设计',
    'init-tag-bug': 'Bug 修复',
    'init-tag-release': '发布',
    'init-tag-tool': '工具',
    'init-tag-schedule': '管理',
    'init-tag-chat': '聊天',
    'init-tag-wiki': '文档',
    'init-tag-qa': '测试',
    'init-tag-sales': '销售',
    'init-tag-arch': '设计',
    'init-tag-sec': '安全',
    'init-tag-data': '分析',
    'init-tag-move': '移动',

    'studio-title': 'QL-Animation Studio (Beta)',
    'sample-select-placeholder': '选择示例动画...',
    'section-metadata': '元数据',
    'label-metadata-lang': '编辑语言',
    'section-config': '配置',
    'section-preview': '预览',
    'label-name': '名称',
    'label-author': '作者',
    'label-description': '描述',
    'label-mode': '模式',
    'label-exclusion-strategy': 'FG 回避',
    'strategy-mask': '接受掩码 (Mask)',
    'strategy-jump': '跳过 FG 区域 (Jump)',
    'strategy-freedom': '全屏自由 (Freedom)',
    'label-rewindable': '可倒带',
    'label-show-metrics': '显示指标',
    'label-show-exclusion': '排除',
    'label-show-canvas': '画布',
    'label-preview-color': '预览颜色',
    'label-speed': '速度',
    'btn-shrink-preview': '缩小预览',
    'btn-expand-preview': '放大预览',
    'label-latency': '延迟',
    'label-density': '密度',
    'label-change-rate': '变化率',
    'label-status': '状态',
    'label-exclusion-area': '排除区域',
    'status-ready': '就绪',
    'status-running': '运行中',
    'status-paused': '已暂停',
    'tooltip-stop': '停止',
    'tooltip-rewind': '倒带',
    'tooltip-play': '播放',
    'tooltip-fast-forward': '快进',
    'tooltip-pause': '暂停',
    'tooltip-eject': '弹出',
    'label-tape-counter': '磁带计数器',
    'btn-download': '下载',
    'btn-upload': '上传',
    'btn-pr': '准备 PR',
    'btn-close': '关闭',
    'pr-guide-title': 'Pull Request 指南',
    'pr-step-1': '将动画保存为 <code>.js</code> 文件并下载。',
    'pr-step-2':
        'Fork <a href="https://github.com/masanori-satake/QuickLog-Solo" target="_blank">QuickLog-Solo</a> 仓库。',
    'pr-step-3': '将文件放入 <code>shared/js/animation/</code>。',
    'pr-step-4': '提交 Pull Request！',
    'pr-note': '您的贡献让这个工具变得更好！',
    'back-to-landing': '返回首页',
    'toast-downloaded-js': '已下载 JS 文件',
    'toast-loaded-json': '已从 JSON 加载',
    'toast-invalid-json': '无效的 JSON',
    'toast-loaded-js': '已从 JS 加载',
    'toast-anim-error': '动画错误: ',
    'btn-toggle-wrap': '自动换行',
    'btn-search': '查找与替换',
    'search-placeholder': '查找...',
    'replace-placeholder': '替换...',
    'btn-replace': '替换',
    'btn-replace-all': '全部替换',
    'section-console': '控制台',
    'btn-clear-console': '清空控制台',

    // Category Editor
    'category-editor-title': '业务分类编辑器',
    'btn-launch-category-editor': '编辑',
    'category-detail': '分类详情',
    'tags-common': '标签(共通)',
    'setting-theme-common': '主题(共通)',
    'setting-animation-by-category-common': '背景动画设置(共通)',
    'section-code-view': 'NDJSON 视图',
    'btn-new-start': '从默认集开始',
    'btn-clear-all': '全部清除',
    'confirm-clear-all': '清除所有分类？是否继续？',
    'confirm-load-default': '将加载默认分类集。当前更改将丢失。是否继续？',
    'toast-import-success': '粘贴成功',
    'toast-import-failed': '粘贴失败：格式无效',
    'toast-export-success': '复制成功',
    'toast-no-categories': '没有可复制的分类',
    'page-break-label': '换页',
    undo: '撤销',
    redo: '重做',
    'tag-box-title': '标签箱',
    'tag-replace-title': '替换标签',
    'tag-replace-before': '替换前',
    'tag-replace-after': '替换后',
    'tooltip-replace-tags': '替换标签',
    'privacy-policy-local-only': '所有处理均在浏览器内完成，不与外部服务器通信。您的信息绝不会泄露到外部。',
    'privacy-policy-local-only-short': '仅限本地',
    'setting-session-sync': '多端同步 (Sync) (β)',
    'setting-session-sync-desc': '通过浏览器同步功能，在不同设备间同步设置、分类和近期历史记录。',
    'setting-session-sync-note': '※ 不支持在不同浏览器（如 Chrome 和 Edge）之间进行同步。',
    'confirm-enable-session-sync': '是否启用多端同步？当前数据将被上传至浏览器的同步存储空间。',
    'confirm-disable-session-sync': '是否禁用多端同步？本设备将停止同步，但与其他设备共享的数据仍将保留。',
    'session-sync-active': '同步已启用',
    'local-only-active': '仅限本地',

    'sync-setup-title': '多端同步设置',
    'sync-setup-desc': '在开始同步之前，请选择现有数据与其他设备数据之间的优先级。',
    'sync-setup-settings-title': '设置与类别同步',
    'sync-setup-history-title': '历史记录同步',
    'sync-setup-cloud-to-local': '使用其他设备的数据覆盖',
    'sync-setup-local-to-cloud': '使用本设备的数据覆盖其他设备',
    'sync-setup-merge': '将本设备的历史记录与其他设备合并',

    'alarm-label-action-category': '启动分类',
    'alarm-label-days-of-week': '星期',
    'alarm-label-day-of-month': '日期',
    'alarm-label-days-before-end': '距离月末天数',
    'alarm-label-confirmation': '需要确认',
    'alert-business-days-min': '请至少选择一个工作日',
    'alarm-editor-select-prompt': '请从左侧选择一个闹钟。',
    'alarm-label-default-name': 'Alarm #{index}',
    'alarm-label-not-initialized': '(Alarm #{index} - Not Initialized)',
    'alarm-section-basic': '基本设置',
    'alarm-section-action': '操作设置',
    'alarm-section-schedule': '计划设置',
    'display-mode-retro-lcd': '复古液晶',
    'display-mode-retro-crt': '显像管显示器',
    'display-mode-retro-nixie': '辉光管',

    'tab-custom-anim': '自定义动画',
    'custom-anim-select-label': '已注册的自定义动画',
    'custom-anim-import': '导入 (新增)',
    'btn-add-from-file': '从文件添加',
    'btn-add-from-clipboard': '从剪贴板添加',
    'custom-anim-export': '导出',
    'btn-save-to-file': '保存到文件',
    'btn-copy-to-clipboard': '复制到剪贴板',
    'custom-anim-delete-label': '删除',
    'btn-delete-selected-anim': '删除所选动画',
    'toast-custom-anim-imported': '自定义动画导入成功',
    'toast-custom-anim-exported': '自定义动画已复制到剪贴板',
    'confirm-delete-custom-anim': '确定要删除自定义动画 "{name}" 吗？\n(分配给此动画的类别将重置为“无”)',
    'alert-invalid-qlanim': '无效的 .qlanim 格式。请检查文件结构。',
    'btn-launch-maker': '启动动画生成器',
    'maker-title': '动画生成器',
    'placeholder-meta-name': '我的动画',
    'placeholder-meta-author': '您的名字',
    'placeholder-meta-desc': '动画的简短说明',
    'maker-overflow-behavior': '水平平铺',
    'maker-max-width': '最大宽度 (maxWidth)',
    'maker-scale-with-height': '按高度缩放',
    'maker-invert': '反转（黑白反转）',
    'maker-brightness': '亮度',
    'maker-gif-import': '导入 GIF 图片',
    'maker-drop-hint': '将动态GIF拖放到此处，或点击选择',
    'maker-file-name': '文件:',
    'maker-dimensions': '尺寸:',
    'maker-frames-count': '帧数:',
    'maker-spec-monitor': '实时规格监视器',
    'maker-current-scale': '当前比例',
    'maker-target-height': 'targetHeight',
    'maker-boundary-height': '辅助线高度 (向导)',
    'maker-drag-instruction': '拖动图片调整位置',
    'maker-zoom-in': '放大',
    'maker-zoom-out': '缩小',
    'maker-playback-zoom': '播放与缩放',
    'maker-data-transfer': '数据传输',
    'maker-warning-title': '警告',
    'maker-note-extension-only': '* 动画生成器 仅在作为浏览器扩展程序安装时可用。在预览版中无法启动。',
    'maker-error-name-empty': '名称不能为空',
    'maker-error-name-duplicate': '已存在同名的自定义动画。请输入不同的名称。',
    'maker-select-prompt': '请添加自定义动画',
    'about-pwa-section-title': '移动 (PWA) 版 QuickLog-Solo (β)',
    'about-pwa-section-desc': '使用智能手机摄像头扫描二维码即可启动 PWA 版本或传输设置。',
    'about-pwa-url-qr-label': '1. PWA 启动网址',
    'about-pwa-settings-qr-label': '2. 导出设置',
    'about-pwa-import-title': '导入 QuickLog-Solo 设置 (β)',
    'about-pwa-import-desc': '扫描 PC 扩展程序中显示的设置二维码以应用类别和闹钟设置。',
    'btn-pwa-scan-qr': '扫描二维码',
    'qr-scan-modal-title': '二维码扫描',
    'qr-scan-status-scanning': '请将摄像头对准二维码',
    'btn-select-qr-image': '从图片选择',
    'toast-settings-imported': '设置已成功导入！',
    'qr-scan-failed-not-found': '未在图片中检测到二维码',
    'qr-scan-camera-error': '摄像头访问被拒绝或不可用。请从图片选择。',
    'qr-scan-unsupported': '此浏览器不支持扫描二维码。请使用支持此功能的浏览器。',
    'qr-scan-invalid-payload': '无效的二维码数据',
};

...
function crossCheckVertical(startX, startY, maxCount, isDark, height, checkRatio) {
    ...
    let y = topY - 1;
    let count1 = 0;
    while (y >= 0 && !isDark(startX, y) && count1 < maxCount) {
        count1++;
        y--;
    }
    let count0 = 0;
    while (y >= 0 && isDark(startX, y) && count0 < maxCount) {
        count0++;
        y--;
    }
    ...
    y = bottomY + 1;
    let count3 = 0;
    while (y < height && !isDark(startX, y) && count3 < maxCount) {
        count3++;
        y++;
    }
    let count4 = 0;
    while (y < height && isDark(startX, y) && count4 < maxCount) {
        count4++;
        y++;
    }
...
function crossCheckHorizontal(startX, startY, maxCount, isDark, width, checkRatio) {
    ...
    let x = leftX - 1;
    let count1 = 0;
    while (x >= 0 && !isDark(x, startY) && count1 < maxCount) {
        count1++;
        x--;
    }
    let count0 = 0;
    while (x >= 0 && isDark(x, startY) && count0 < maxCount) {
        count0++;
        x--;
    }
    ...
    x = rightX + 1;
    let count3 = 0;
    while (x < width && !isDark(x, startY) && count3 < maxCount) {
        count3++;
        x++;
    }
    let count4 = 0;
    while (x < width && isDark(x, startY) && count4 < maxCount) {
        count4++;
        x++;
    }
...
function scanImageDataPureJS(canvasOrImageData) {
    ...
    const blocks = Array.from(
        { length: numBlocks },
        (_, b) => new Uint8Array(b < shortBlocksCount ? blockSize : blockSize + 1)
    );

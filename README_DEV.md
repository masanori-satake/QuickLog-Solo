...
- **ウィンドウの退避:** ピン留め開始時、元のウィンドウを画面外（`-10000, -10000`）に移動し、サイズを最小化します。これにより、作業の邪魔になるのを防ぎます。
- **状態の復元:** PiP ウィンドウが閉じられた際、元の座標とサイズを復元します。
- **配置の安全性:** `js/app.js` の `recoverWindowPosition` により、起動時にウィンドウが画面外にある場合は、自動的に視認可能な位置（`100, 100`）に戻します。これはクラッシュ等で復元処理がスキップされた場合の備えです。
